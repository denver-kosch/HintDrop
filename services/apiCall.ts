...
async function apiCall<T=unknown>(api: string, { method = "GET", body = {}, headers = {}, auth = true, signal}: ApiRequestOptions = {}) : Promise<T>{
	...
	if (!isFormData && !finalHeaders.has("Content-Type")) finalHeaders.set("Content-Type", "application/json");
